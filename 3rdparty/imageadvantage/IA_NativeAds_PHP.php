...
function getImageFileName($AdvertiserName, $SearchString, $ImageStyle)
{
	...
	$hashName = substr($hashName,0,1) . "/" . substr($hashName,1,2) . "/" . substr($hashName,3,2) . "/" . substr($hashName,5,27) . ".png";
	return $hashName;
}
...
function GetIAProductImageURL($pid, $pSearchString, $pTitle, $pDescription, $pAdvertiser)
{
  $pDisplayURL = $pAdvertiser;
	...
	$FullURL = "http://cr0.worthathousandwords.com/N/" . $ImgFileName . "?pid=" . $pid . "&qs=" . $lQSEnc . "&d=" . urlencode($pDisplayURL);
	return $FullURL;
}
